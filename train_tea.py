...
def eval(model, eval_features,args,device,n_gpu):
    ...
    for batch in eval_dataloader:
        ...
        with torch.no_grad():
            loss,logits= model(input_ids, segment_ids, input_mask, label_ids,training=False)
        if n_gpu > 1:
...
def main():
    ...
    train_features = convert_examples_to_features(args.small, train_examples, label_list, args.max_seq_length,tokenizer, logger=logger,cache_file=os.path.join(args.data_dir,"train.pkl"),num=640)
    ...
    train_dataloader = DataLoader(train_features, sampler=train_sampler, collate_fn=collate_fn,batch_size=args.train_batch_size)
    ...
    for epochs in range(int(args.num_train_epochs)):
        ...
        model.train()
        logger.info("step {}/{}, eval_acc:{:.4f}".format(global_step, num_train_steps, epochs, eval_acc))
